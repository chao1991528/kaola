...
class Common extends Api
{
    ...
    public function upload()
    {
        ...
        // halt(count($files));
        if (empty($files)) {
            $this->error(__('No file upload or server upload limit exceeded'), null, StatusCode::CODE_UPLOAD_ERROR);
        }
        $data = [];
...
//                    }
                ...
                $attachment->isUpdate(false)->save();
                \think\Hook::listen("upload_after", $attachment);
                $data['url'][] = config('image_domain') . $uploadDir . $splInfo->getSaveName();
            } else {
                // 上传失败获取错误信息
