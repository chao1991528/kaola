<?php

namespace app\admin\model;

use think\Model;

class LiveCategory extends Model
{
    // 表名
    protected $name = 'live_category';

    // 自动写入时间戳字段
    protected $autoWriteTimestamp = false;

    // 定义时间戳字段名
    protected $createTime = false;
    protected $updateTime = false;

    // 追加属性
    protected $append = [
        'category_type_text',
        'add_time_text',
        'delete_time_text'
    ];



    public function getCategoryTypeList()
    {
        return ['house' => __('House'),'live' => __('Live')];
    }


    public function getCategoryTypeTextAttr($value, $data)
    {
        $value = $value ? $value : (isset($data['category_type']) ? $data['category_type'] : '');
        $list = $this->getCategoryTypeList();
        return isset($list[$value]) ? $list[$value] : '';
    }


    public function getAddTimeTextAttr($value, $data)
    {
        $value = $value ? $value : (isset($data['add_time']) ? $data['add_time'] : '');
        return is_numeric($value) ? date("Y-m-d H:i:s", $value) : $value;
    }


    public function getDeleteTimeTextAttr($value, $data)
    {
        $value = $value ? $value : (isset($data['delete_time']) ? $data['delete_time'] : '');
        return is_numeric($value) ? date("Y-m-d H:i:s", $value) : $value;
    }

    protected function setAddTimeAttr($value)
    {
        return $value && !is_numeric($value) ? strtotime($value) : $value;
    }

    protected function setDeleteTimeAttr($value)
    {
        return $value && !is_numeric($value) ? strtotime($value) : $value;
    }


}
