	...
	$.fn.select2tree = function(options) {
		...
		opts.templateResult = function(data, container) {
			if(data.element) {
				...
				if($element.attr("isparent")) {
					$(container).attr("isparent", "true");
	...
	function open() {
		setTimeout(function() {
			...
			$(".parent").mousedown(function() {
				switchAction($(this).attr("val"), $(this).children('.switch').hasClass("glyphicon-chevron-right"));
				event.stopPropagation();
			});
