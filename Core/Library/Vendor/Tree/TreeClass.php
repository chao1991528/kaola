...
class Tree {
     ...
     *      )
     */
    public function init($arr = array(), $pid = 'pid') {
        $this->arr = $arr;
        $this->ret = '';
        $this->pid_column_name = $pid;
        return is_array($arr);
    }
    ...
    public function get_parent($myid) {
        ...
        if (!isset($this->arr[$myid]))
            return false;
        $pid = $this->arr[$myid][$this->pid_column_name];
        $pid = $this->arr[$pid][$this->pid_column_name];
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if ($a[$this->pid_column_name] == $pid)
                    $newarr[$id] = $a;
            }
        }
        return $newarr;
    ...
    public function get_child($myid) {
        ...
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if ($a[$this->pid_column_name] == $myid)
                    $newarr[$id] = $a;
            }
        }
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                @extract($value);
                $pid_column = $this->pid_column_name;
                $$pid_column == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
